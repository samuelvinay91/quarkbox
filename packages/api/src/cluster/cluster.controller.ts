...
export class ClusterController {
  ...
  async createCluster(@Body() dto: CreateClusterDto) {
    return this.clusterService.createCluster(dto);
  }

  @Post(':id/stop')
  @ApiOperation({ summary: 'Stop all sandboxes in a cluster' })
  @ApiParam({ name: 'id', type: 'string' })
  async stopCluster(@Param('id', ParseUUIDPipe) id: string) {
    return this.clusterService.stopCluster(id);
  }
  ...
  @ApiParam({ name: 'id', type: 'string' })
  async destroyCluster(@Param('id', ParseUUIDPipe) id: string) {
    return this.clusterService.destroyCluster(id);
  }
}
