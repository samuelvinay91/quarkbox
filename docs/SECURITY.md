...
- Programmatic access (SDK/CLI) uses API keys generated by `POST /api/auth/api-key`.
- Keys are formatted `qbk_<64 hex chars>` (the separate auth service also issues `qb_<base64url>` keys in some flows). Text notes in the code indicate these should be **stored hashed**: `ApiKeyService.generate()` computes a **SHA-256 hash** of the key and persists only the hash + a `keyPrefix` (`qbk_` + first 4 hex chars after prefix), never the raw key in the API-key repository.
